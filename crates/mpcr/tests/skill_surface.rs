...
fn skill_router_restores_dispatch_and_recursive_reports() -> anyhow::Result<()> {
    ...
    ensure!(skill_md.contains("packaged Rust binary"));
    ensure!(skill_md.contains("just dist-host"));
...
#[test]
fn shipped_wrapper_help_runs_when_dist_binary_is_present() -> anyhow::Result<()> {
    let root = skill_root()?;
    let wrapper = root.join("scripts/mpcr");
    let dist_bin = root.join("dist/linux-x86_64/mpcr");
    if !dist_bin.exists() {
        return Ok(());
    }
...
#[test]
fn shipped_wrappers_use_dist_layout() -> anyhow::Result<()> {
    let root = skill_root()?;
...
#[cfg(unix)]
#[test]
fn shipped_wrapper_execs_staged_dist_binary() -> anyhow::Result<()> {
    let root = tempdir()?;
    let scripts_dir = root.path().join("scripts");
    let dist_dir = root.path().join("dist/linux-x86_64");
    ...
    fs::copy(env!("CARGO_BIN_EXE_mpcr"), dist_dir.join("mpcr"))?;
    let mut fake_bin_perms = fs::metadata(dist_dir.join("mpcr"))?.permissions();
    fake_bin_perms.set_mode(0o755);
    fs::set_permissions(dist_dir.join("mpcr"), fake_bin_perms)?;
