...
Rust is one of the easiest ecosystems to support for staged binaries because release outputs land in a predictable place.
...
`cargo fetch` is a good minimal bootstrap. `cargo build` is acceptable when the team wants a stronger readiness check.
