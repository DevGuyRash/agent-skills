...
Stop short when the missing piece would change repository policy, not just one command. Examples:
...
Use placeholders as a deliberate fallback when you can name the lifecycle stage but cannot defend the exact command.
...
3. add `local-dist`, `committed-dist`, or `cross-os-dist` only when the repo truly has a distribution story
