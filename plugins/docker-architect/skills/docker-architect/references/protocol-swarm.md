...
- Init-sidecar startup sequencing is **Compose-only** — do not include `*-init-perms` services in stack files.
- Flat resource keys (`cpus`, `mem_limit`, `pids_limit`) are **Compose-only** — use `deploy.resources.limits` and `deploy.resources.reservations` instead.
...
Emit `docker-stack.yaml` only after all prior gates pass. Use `deploy.resources.limits` for resource constraints, `deploy.restart_policy` for restart behavior, and explicit overlay networks.
...
Include traceability IDs (AC-_, IMG-_, RSK-_, O-_) in every major section.
...
For non-root stateful services that need writable named volumes, document and require a pre-deploy step:
