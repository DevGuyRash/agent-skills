...
A task-value claim requires decision-linked behavioral evidence after deterministic inspection shows that the claim remains material. Use [task-value evidence](references/task-value-evidence.md) for that decision. Explicit task trials do not prove implicit activation; use [trigger evidence](references/trigger-evals.md) when retrieval or coexistence is at issue.
