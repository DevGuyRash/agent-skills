...
Skill Auditor SHALL reject a mismatched `claim_id`, request digest, omitted closure condition, irrelevant tested condition, unsupported generalization, or evidence reference that does not bear on the claim. JSON Schema alone cannot establish ID uniqueness or exact request/result coverage; when Split Testing's deterministic `split-test exchange verify REQUEST.json RESULT.json` check is available, use it before the audit-owned relevance and disposition judgment. A valid return remains evidence input to the audit; Skill Auditor retains rightful authority interpretation, materiality, severity, repair, release, reopening, and the final disposition.
