...
- `plan` and per-surface `compare` can include `pq`, `connections`, and `model` in the package path so callers can see capability and drift without invoking COM mutation.
- Package-backed `sync` treats `pq`, `connections`, and `model` as `desktop-write` surfaces: it inventories, plans, and compares them without invoking desktop Excel, then routes mutation to the desktop backend.
