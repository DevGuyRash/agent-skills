...
4. Run `matrix-audit` when you need the same audit flow across several copied workbooks with one aggregate summary.

## Notes

- Use repo `.local` output roots when the work is repo-local and worth preserving.
- Generic audit always copies the workbook before mutating it.
- Raw parity preserves the original extraction surfaces.
- Normalized parity filters clearly internal Excel-generated names such as `_xlfn.*`, `_xlpm.*`, and `_xlws.*`.
