...
For package-readable `.xlsx` and `.xlsm` workbooks, the package helper still supports portable `inspect`, `query`, `bootstrap`, `plan`, per-surface `compare`, and safe-surface `sync` without live COM. COM is still required for legacy mutation flows, `.xls`, `.xlsb`, and the live Excel-only surfaces.
